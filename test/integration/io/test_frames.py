...
def test_tiff_tensor(tmpdir):
    """
    Checks TiffTensor on real generated data dumped on disk and compares it to tensor
    in memory.
    """
    ...
    data_tensor = frames.TiffTensor(fname)

    assert data_tensor.size() == torch.Size([1000, 64, 64])
