...
if __name__ == '__main__':
    ...
    logger = SummaryWriter(log_dir, comment='multi_frame')

    if len(sys.argv) == 1:  # no .ini file specified
        dataset_file = deepsmlm_root + \
                       'data/2019-03-19/complete_z_range.npz'
        weight_out = deepsmlm_root + 'network/2019-03-22/model_multiframe_small_batch.pt'
        weight_in = None  # deepsmlm_root + 'network/2019-03-22/model_multiframe_2.pt'
    ...
    if mode == 'PreComputedSamples':
        ...
        train_loader = DataLoader(train_data_smlm, batch_size=64, shuffle=False, num_workers=12, pin_memory=True)
    ...
    model = DenseLoco(extent=psf_extent, ch_in=3, dim_out=3)
    ...
    """Log the model"""
    dummy = torch.rand((32, 3, img_shape[0], img_shape[1]), requires_grad=True)
