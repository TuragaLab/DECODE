...
class UniformizeOffset:
    ...
    def __init__(self, n_bins: int):
        """
        Args:
            n_bins (int): The bias scales with the uncertainty of the localization.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_sigma = x[:, 6]
        y_sigma = x[:, 7]
        x_sigma_var = torch.var(x_sigma[active_px])
        y_sigma_var = torch.var(y_sigma[active_px])
        weighted_sig = x_sigma ** 2 + (torch.sqrt(x_sigma_var / y_sigma_var) * y_sigma) ** 2
        weighted_sig = np.where(x[:, 0] > self.p_th, weighted_sig, 0)
