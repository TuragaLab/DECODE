...
challenge_import = {
    ...
    "px_size": (100.0, 100.0),
    "xyz_axis": (1, 0, 2),
    "xyz_nm_factor": (1.0, 1.0, -1.0),
    "xyz_nm_shift": (-150.0, -50.0, 0.0),
    "xyz_px_factor": None,
    "xyz_px_shift": None,
    "frame_ix_shift": -1,
}
...
def transform_emitter(em: EmitterSet, trafo: dict) -> EmitterSet:
    ...
    # set Px Size
    mod_em.px_size = (
        torch.tensor(trafo["px_size"])
        if trafo["px_size"] is not None
        else mod_em.px_size
    )
    ...
    if trafo["xyz_axis"] is not None:
        ...
        if em.xyz_cr is not None:
            mod_em.xyz_cr = mod_em.xyz_cr[:, trafo["xyz_axis"]]
        if em.xyz_sig is not None:
