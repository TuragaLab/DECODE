...
class Renderer2D(Renderer):
    ...
    def get_extent(self, em):
        ...
        xextent = (em.xyz_nm[:, 0].min(), em.xyz_nm[:, 0].max()
                   ) if self.xextent is None else self.xextent
        yextent = (em.xyz_nm[:, 1].min(), em.xyz_nm[:, 1].max()
                   ) if self.yextent is None else self.yextent
        zextent = (em.xyz_nm[:, 2].min(), em.xyz_nm[:, 2].max()
                   ) if self.zextent is None else self.zextent
    ...
    def render(self, em: emitter.EmitterSet, col_vec=None, ax=None):
        ...
        if col_vec is not None:
            ...
            colb = mpl.colorbar.ColorbarBase(
                cax, cmap=plt.get_cmap('jet'), values=np.linspace(
                    0, 1., 101), norm=mpl.colors.Normalize(
                    0., 1.))
    ...
    def forward(self, em: emitter.EmitterSet, col_vec=None) -> torch.Tensor:
        ...
        ind_mask = (em.xyz_nm[:,
                              0] > xyz_extent[0][0]) * (em.xyz_nm[:,
                                                                  0] < xyz_extent[0][1]) * (em.xyz_nm[:,
                                                                                                      1] > xyz_extent[1][0]) * (em.xyz_nm[:,
                                                                                                                                          1] < xyz_extent[1][1]) * (em.xyz_nm[:,
                                                                                                                                                                              2] > xyz_extent[2][0]) * (em.xyz_nm[:,
                                                                                                                                                                                                                  2] < xyz_extent[2][1])
        ...
        if col_vec is not None:
            ...
            self.colextent = (col_vec.min(), col_vec.max()
                              ) if self.colextent is None else self.colextent
            ...
            if self.sigma_blur:
                RGB = np.array([gaussian_filter(RGB[:, :, i], sigma=[self.sigma_blur /
                                                                     self.px_size, self.sigma_blur /
                                                                     self.px_size]) for i in range(3)]).transpose(1, 2, 0)
    ...
    def hist2d(self, em, col_vec, x_hist_ext, y_hist_ext, c_range=None):
        ...
        if col_vec is not None:
            ...
            col_hist, _, _ = np.histogram2d(xy[:, 0], xy[:, 1], bins=(
                hist_bins_x, hist_bins_y), weights=c_weight)
...
class RendererIndividual2D(Renderer2D):
    ...
    def place_gaussians(int_hist, inds, W, fs):
        for i in range(len(W)):
            int_hist[inds[i, 1]:inds[i, 1] + fs,
                     inds[i, 0]:inds[i, 0] + fs] += W[i]
        return int_hist

    @script
    def place_gaussians_weighted(comb_hist, inds, weights, W, fs):
        for i in range(len(W)):
            comb_hist[inds[i, 1]:inds[i, 1] + fs, inds[i, 0]:inds[i,
                                                                  0] + fs] += torch.stack([W[i], W[i] * weights[i]], -1)
        return comb_hist
    ...
    def hist2d(self, em, col_vec, x_hist_ext, y_hist_ext, c_range=None):
        ...
        s_inds = ((xy_mus -
                   torch.Tensor([x_hist_ext[0], y_hist_ext[0]]).to(self.device)) //
                  self.px_size).type(torch.LongTensor)

        if col_vec is not None:

            c_pos = torch.clip(col_vec, c_range[0], c_range[1])
            c_weight = ((c_pos - c_range[0]) /
                        (c_range[1] - c_range[0])).to(self.device)
            ...
            for i in tqdm(range(len(xy_mus) // self.bs)):
                ...
                c_ws = c_weight[sl]
                comb_hist = self.place_gaussians_weighted(
                    comb_hist, sub_inds, c_ws, W, torch.tensor(self.fs))

            comb_hist = comb_hist[self.fs // 2:-
                                  (self.fs // 2 + 1), self.fs // 2:-(self.fs // 2 + 1)]
            ...
            for i in tqdm(range(len(xy_mus) // self.bs)):
                ...
                sub_inds = s_inds[sl]
                W = self.calc_gaussians(xy_mus[sl], xy_sigs[sl], mesh)
                int_hist = self.place_gaussians(
                    int_hist, sub_inds, W, torch.tensor(self.fs))

            int_hist = int_hist[self.fs // 2:-
                                (self.fs // 2 + 1), self.fs // 2:-(self.fs // 2 + 1)]
