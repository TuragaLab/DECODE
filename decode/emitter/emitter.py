...
class EmitterSet:
    ...
    def _set_typed(
        ...
        if frame_ix.dtype not in (torch.int16, torch.int32, torch.int64):
            raise ValueError(
                f"Frame index must be integer type and not {frame_ix.dtype}."
            )
        ...
        xyz = (
            xyz
            if xyz.shape[1] == 3
            else torch.cat((xyz, torch.zeros_like(xyz[:, [0]])), 1)
        )
        ...
        if num_input != 0:
            ...
            self.prob = (
                prob.type(f_type)
                if prob is not None
                else torch.ones_like(frame_ix).type(f_type)
            )
            self.bg = (
                bg.type(f_type)
                if bg is not None
                else float("nan") * torch.ones_like(frame_ix).type(f_type)
            )

            self.xyz_cr = (
                xyz_cr.type(f_type)
                if xyz_cr is not None
                else float("nan") * torch.ones_like(self.xyz)
            )
            self.phot_cr = (
                phot_cr.type(f_type)
                if phot_cr is not None
                else float("nan") * torch.ones_like(self.phot)
            )
            self.bg_cr = (
                bg_cr.type(f_type)
                if bg_cr is not None
                else float("nan") * torch.ones_like(self.bg)
            )

            self.xyz_sig = (
                xyz_sig.type(f_type)
                if xyz_sig is not None
                else float("nan") * torch.ones_like(self.xyz)
            )
            self.phot_sig = (
                phot_sig.type(f_type)
                if phot_sig is not None
                else float("nan") * torch.ones_like(self.phot)
            )
            self.bg_sig = (
                bg_sig.type(f_type)
